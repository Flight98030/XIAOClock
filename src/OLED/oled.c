...
void OLED_WR_Byte(u8 dat,u8 mode)
{
	...
	i2c_master_write_byte(cmd, 0x78, ACK_CHECK_ENABLE);
	if(mode){
		i2c_master_write_byte(cmd, 0x40, ACK_CHECK_ENABLE);}
	else{
		i2c_master_write_byte(cmd, 0x00, ACK_CHECK_ENABLE);}
    i2c_master_write_byte(cmd, dat, ACK_CHECK_ENABLE);
    i2c_master_stop(cmd);
    i2c_master_cmd_begin(IIC_CTRL_NUM, cmd, 1000 / portTICK_RATE_MS);
...
void OLED_Refresh(void)
{
	...
	for(i=0;i<8;i++)
	{
		...
		for(n=0;n<128;n++)
		{
			i2c_master_write_byte(cmd, OLED_GRAM[n][i], ACK_CHECK_ENABLE);
		}
		i2c_master_stop(cmd);
    	i2c_master_cmd_begin(IIC_CTRL_NUM, cmd, 10000 / portTICK_RATE_MS);
    	i2c_cmd_link_delete(cmd);
  }
}
//清屏函数
